...
return {
	...
	opts = {
		files = {
			-- Pin fd explicitly so we never fall back to BSD find on macOS.
			cmd = "fd --type f --type l --hidden --follow --exclude .git",
		},
		grep = {
			-- Pin ripgrep explicitly so live_grep never falls back to system grep.
